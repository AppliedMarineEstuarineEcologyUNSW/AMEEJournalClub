...
plot(sydney, lwd=0.6)
points(time1, pch=16, cex=2, col=cols[1])
points(time2, pch=16, cex=2, col=cols[2])
points(time3, pch=16, cex=2, col=cols[3])
title('Sampling sites in Sydney Harbour', cex=0.8)
legend( "bottomleft", legend=c("Time One","Time Two","Time Three"),cex=2, col=cols, pch=16,bty="n")
box()
...
plot(sample_point, add=T, cex=2, col='red')

sampling_radius<-gBuffer(sample_point, width=60, byid=F)
plot(sampling_radius,add=T, lty=1,lwd=3)
